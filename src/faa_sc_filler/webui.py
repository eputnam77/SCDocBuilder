...
def generate(
    template: str, worksheet: str, dry_run: bool = False
...
def validate_inputs(cfr_part: str, docket_no: str, notice_no: str) -> Dict[str, str]:
    ...
    if cfr_part:
        ...
        if not all(p in valid_parts for p in parts):
            errors["cfr_part"] = (
                "Invalid CFR Part(s). Use comma-separated values from: 23,25,27,29,31,33,35"
            )
...
def enhanced_generate(
    ...
    try:
        ...
        if cfr_part:
            worksheet_data["{CFRPart}"] = cfr_part
        if docket_no:
            worksheet_data["{DocketNo}"] = docket_no
        if notice_no:
            worksheet_data["{NoticeNo}"] = notice_no
...
def handle_generation(*args):
    ...
    try:
        ...
        try:
            ...
            if cfr:
                worksheet_data["{CFRPart}"] = cfr
            if docket:
                worksheet_data["{DocketNo}"] = docket
            if notice:
                worksheet_data["{NoticeNo}"] = notice
...
def create_ui() -> gr.Blocks:
    ...
    with gr.Blocks(title="FAA Special Conditions Template Filler") as demo:
        ...
        template_file = gr.File(
            ...
            file_types=[".docx"],
            type="binary",
            elem_id="template-file",
        )
        worksheet_file = gr.File(
            label="Worksheet (.docx)",
            file_types=[".docx"],
            type="binary",
            elem_id="worksheet-file",
        )
        ...
        with gr.Row():
            cfr_part = gr.Textbox(
                label="CFR Part(s) (comma-separated)", placeholder="25,27"
            )
        ...
        Use the **Tab** key to navigate between fields and the **Enter** key to submit.
        """
        )
    return demo
