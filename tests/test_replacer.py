...
def test_multiple_replacements_in_run():
    ...
    replacer = PlaceholderReplacer()
    replacements = {"{SMEName}": "John Doe", "{SMEEmail}": "john@example.com"}
...
def test_partial_placeholder():
    ...
    doc = Document()
    p = doc.add_paragraph(
        "Contact: {SMEName, Phone: SMEPhone}"
    ...
    replacer.process_paragraph(p, replacements)
    assert (
        p.text == "Contact: {SMEName, Phone: SMEPhone}"
    )  # Should not modify malformed placeholders
