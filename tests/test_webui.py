...
import re
import gradio as gr
from faa_sc_filler.webui import generate, create_ui, handle_generation
...
async def test_ui_creation():
    ...
    assert (
        len([c for c in components if isinstance(c, gr.File)]) >= 2
    )  # Template and worksheet
...
def test_enhanced_generate(sample_files):
    ...
    result = generate(template_file, worksheet_file, dry_run=False)
...
def test_handle_generation_preserves_download(tmp_path):
    ...
    try:
        ...
        result = handle_generation(
            template_bytes, worksheet_bytes, None, None, None, False
        )
        assert len(result) == 3
        path, diff, show_message = (
            result  # Renamed to show_message to better reflect its purpose
        )
