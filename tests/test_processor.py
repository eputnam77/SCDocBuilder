...
def test_multiple_replacements_same_paragraph():
    ...
    processor = DocumentProcessor()
    replacements = {"{SMEName}": "John Doe", "{SMEPhone}": "555-0123"}
...
def test_summary_paragraph_processing():
    ...
    assert (
        paragraph.runs[1].text == "Testing autopilot in aircraft"
    ), "Content should be in sentence case"
...
def test_regex_pattern_handling():
    ...
    processor = DocumentProcessor()
    replacements = {"{Complex.Field}": "Complex Value", "{Simple}": "Simple Value"}
